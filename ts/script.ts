...
let groups: object
(async () => { groups = (await getGroups()).groups })();
...
function switchDarkThemeHandler() {
    ...
    const nodes = {
        ...
        autocomplete: document.getElementsByClassName("autocomplete") as HTMLCollectionOf<HTMLElement>,
        autocompleteItem: document.getElementsByClassName("autocomplete-item") as HTMLCollectionOf<HTMLElement>,
    }
    ...
    if (theme == "light") {
        ...
        for (const node of nodes["autocomplete"]) node.classList.remove("autocomplete-dark")
        for (const node of nodes["autocompleteItem"]) node.classList.remove("autocomplete-item-dark")
        ...
        for (const node of nodes["autocomplete"]) node.classList.add("autocomplete-dark")
        for (const node of nodes["autocompleteItem"]) node.classList.add("autocomplete-item-dark")
...
async function getGroups() {
    try {
        const response = await fetch("https://itcpd.ru/api/V1/getGroups/?format=json")
...
async function setGroupsAutocomplete(groupName: string = "") {
    const groupAutocompleteItems = document.querySelector(".autocomplete-items") as HTMLElement
    const groupAutocomplete = document.querySelector(".group-autocomplete") as HTMLElement
    const switchTheme = document.querySelector("#flexSwitchCheckDarkTheme") as HTMLInputElement
    ...
    for (const group in groups) {
        if (group.split(/\-|\s/).join("").toLowerCase().includes(groupName.split(/\-|\s/).join("").toLowerCase())) {
            ...
            if (theme == "dark") listItem.classList.add("autocomplete-item-dark")
...
function groupAutocompleteHandler(event: Event) {
    const groupAutocomplete = document.querySelector(".group-autocomplete") as HTMLElement
    const groupInput = document.querySelector(".group-input") as HTMLInputElement
...
function setGroupHandler(event: Event) {
    const groupInput = document.querySelector(".group-input") as HTMLInputElement
    const groupAutocomplete = document.querySelector(".group-autocomplete") as HTMLElement
    const target = event.target as HTMLElement

    if (target.tagName === "LI") {
        groupInput.value = target.textContent ?? ""
...
function bodyHandler(event: Event) {
    const list: HTMLElement[] = Array.from(document.getElementsByTagName("ul"))
    const target = event.target as HTMLElement
...
function setScheduleHandler(event: KeyboardEvent) {
    if (event.key == "Enter") {
        const groupInput = document.querySelector(".group-input") as HTMLInputElement
        for (const group in groups) {
...
window.onload = function () {
    ...
    const groupAutocomplete = document.querySelector(".group-input-group") as HTMLInputElement
    if (groupAutocomplete) groupAutocomplete.onclick = groupAutocompleteHandler
    if (groupAutocomplete) groupAutocomplete.onfocus = groupAutocompleteHandler
    if (groupAutocomplete) groupAutocomplete.oninput = groupAutocompleteHandler
    if (groupAutocomplete) groupAutocomplete.onkeyup = setScheduleHandler

    const autocompleteItems = document.querySelector(".autocomplete-items") as HTMLInputElement
    if (autocompleteItems) autocompleteItems.onclick = setGroupHandler

    // Closing all drop-down lists
    const body = document.querySelector(".body") as HTMLElement
    if (body) body.onclick = bodyHandler
}
